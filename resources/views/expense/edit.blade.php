...
@section('content')
    ...
    <form class="bg-white shadow-md rounded px-4 pt-6 pb-8 m-1" method="POST" action="{{ route( 'expense.update', $expense->id ) }}">
        ...
        <div class="mb-4">
            ...
            <input value="{{ Carbon\Carbon::parse($expense->expense_date)->format('Y-m-d\TH:i:s') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="expense_category" type="datetime-local" name="expense_date">
            @error('expense_date')
                <div class="text-red-600 font-semibold text-sm py-1"> {{ $message }} </div> 
            @enderror
        </div>
